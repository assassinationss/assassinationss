<div align="center">

# 🔎 Sol Research Desk

**China sourcing intelligence, verification systems & negotiation tools for e-commerce founders.**

🌐 Read everything at **[assassinationss.github.io](https://assassinationss.github.io/)** — the Sol Research Desk hub: 4 free deep guides + the Playbook.

I publish the systems that keep importers from getting burned — open-source toolkits, checklists, and field-tested templates.

</div>

---

## 📕 Products & Tools

| | What it is | Link |
|---|-----------|------|
| 🇨🇳 | **The China Sourcing Playbook 2026** — 25 templates to verify suppliers, negotiate like a local & never get burned | [Product page](https://assassinationss.github.io/china-sourcing-playbook/) |
| 🧰 | **Awesome China Sourcing** — open-source supplier verification toolkit: VERIFY system, red flags, Incoterms, free email templates | [Repo](https://github.com/assassinationss/awesome-china-sourcing) |

## 📌 Start here

New to sourcing from China? Read [the VERIFY system](https://github.com/assassinationss/awesome-china-sourcing#-the-verify-system) first — six steps that filter out 95% of bad suppliers before money moves.

## 🆕 Now shipping

- **[v1.0.0 released](https://github.com/assassinationss/awesome-china-sourcing/releases/tag/v1.0.0)** — the complete verification toolkit, tagged & versioned
- **🧮 New: [Interactive Landed Cost Calculator](https://assassinationss.github.io/calculator.html)** — FOB + freight + duty + VAT, the true per-unit cost
- **5 deep guides** — [Verify a supplier (6 steps)](https://github.com/assassinationss/awesome-china-sourcing/blob/main/docs/how-to-verify-a-chinese-supplier-2026.md) · [Supplier phrases bilingual decoder](https://github.com/assassinationss/awesome-china-sourcing/blob/main/docs/what-chinese-suppliers-say-vs-what-they-mean.md) · [Read a Chinese business license](https://github.com/assassinationss/awesome-china-sourcing/blob/main/docs/how-to-read-a-chinese-business-license.md) · [Pay safely (instruments & scam patterns)](https://github.com/assassinationss/awesome-china-sourcing/blob/main/docs/how-to-pay-a-chinese-supplier-safely-2026.md) · [Alibaba vs 1688](https://github.com/assassinationss/awesome-china-sourcing/blob/main/docs/alibaba-vs-1688-2026.md)
- **[Weekly Q&A](https://github.com/assassinationss/awesome-china-sourcing/discussions/2)** — ask anything about sourcing from China, I answer in-thread
- ⚡ **Launch offer: $12 for the first 20 seats** (then $19) — email `assassinationss@163.com` with subject **Playbook**

## ✍️ Latest writing

- **[Alibaba vs 1688 in 2026: Where Smart Importers Actually Buy](https://dev.to/solresearchdesk/alibaba-vs-1688-in-2026-where-smart-importers-actually-buy-5boi)** — the honest comparison: prices, MOQs, scam recovery
- **[How to Read a Chinese Business License (and Spot a Fake Supplier)](https://dev.to/solresearchdesk/how-to-read-a-chinese-business-license-and-spot-a-fake-supplier-5f5c)** — the line-by-line decoding of the single highest-leverage document in sourcing
- **[How to Verify a Chinese Supplier: The 6-Step System](https://dev.to/solresearchdesk/how-to-verify-a-chinese-supplier-the-6-step-system-before-the-wire-2101)** — the VERIFY framework, start to finish
- **[The GitHub-Only Monetization Stack](https://dev.to/solresearchdesk/the-github-only-monetization-stack-free-repo-to-paid-product-3ahk)** — how this product line runs on GitHub alone

## 📬 Contact

Open an issue on any repo, or reach me at `assassinationss@163.com`.

---

<div align="center"><sub>Star ⭐ the toolkit if it saved you from a bad supplier.</sub></div>
